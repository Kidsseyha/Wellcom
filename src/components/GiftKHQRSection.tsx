    ...
    <section id="khqr-section" className="py-8 px-4 text-center">
      ...
      >
        ...
        <p
          ...
          className={`text-xs font-khmer max-w-xs mx-auto mb-6 ${theme === 'light' ? 'opacity-75' : 'opacity-85'}`}
        >
          {language === 'kh'
        ...
        <div className={`inline-flex p-1 rounded-xl ${theme === 'light' ? 'bg-amber-100/60 border-amber-300' : 'bg-black/60 border-amber-500/30'} border mb-6`}>
          <button
            onClick={() => setCurrency('USD')}
            className={`px-5 py-2 rounded-lg text-xs font-bold transition-all ${
              currency === 'USD'
                ? 'bg-amber-400 text-amber-950 shadow-md'
                : theme === 'light' ? 'text-amber-800 hover:text-amber-900' : 'text-amber-200/70 hover:text-amber-100'
            }`}
          >
            USD ($)
          </button>
        ...
        <div className={`p-6 sm:p-8 rounded-3xl ${theme === 'light' ? 'bg-gradient-to-b from-amber-50 via-white to-amber-50 border-amber-300 shadow-[0_8px_30px_rgba(0,0,0,0.08)]' : 'bg-gradient-to-b from-amber-950/40 via-black/70 to-black/80 border-amber-500/50 shadow-2xl'} border-2 max-w-sm sm:max-w-md mx-auto transition-all`}>
          {/* KHQR Header Banner */}
          <div className={`flex items-center justify-between pb-3.5 mb-4 border-b ${theme === 'light' ? 'border-amber-300/60' : 'border-amber-500/30'}`}>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-red-600 inline-block animate-pulse shadow-sm" />
              <span className={`text-sm font-black tracking-widest ${theme === 'light' ? 'text-red-600' : 'text-red-400'} font-mono`}>KHQR</span>
            </div>
            <span className={`text-xs font-bold px-3 py-1 rounded-lg ${theme === 'light' ? 'bg-amber-100 text-amber-800 border-amber-300' : 'bg-amber-400/20 text-amber-300 border-amber-400/40'} border shadow-sm`}>
              {currency}
            </span>
          </div>

          {/* QR Image Container (Larger width & height) */}
          <div className={`p-4 sm:p-5 bg-white rounded-2xl sm:rounded-3xl shadow-2xl mx-auto mb-5 w-64 h-64 sm:w-72 sm:h-72 flex items-center justify-center border-2 ${theme === 'light' ? 'border-amber-300' : 'border-amber-300/70'} relative overflow-hidden`}>
            <img
              src={currentQr}
              alt={`KHQR ${currency}`}
          ...
          <div className="text-center mb-5">
            <h4 className={`text-base sm:text-lg font-bold font-moul ${theme === 'light' ? 'text-amber-950' : 'text-amber-200'} tracking-wide`}>
              {accountName}
            </h4>
          ...
          {/* Action buttons */}
          <div className={`flex items-center gap-3 pt-3 border-t ${theme === 'light' ? 'border-amber-300/60' : 'border-amber-500/30'}`}>
            <button
              onClick={handleCopy}
              className={`flex-1 py-2.5 px-4 rounded-xl text-xs sm:text-sm font-semibold font-khmer ${theme === 'light' ? 'bg-amber-100/50 hover:bg-amber-200/50 text-amber-800 border-amber-300' : 'bg-amber-950/60 hover:bg-amber-900/60 text-amber-200 border-amber-500/40'} border flex items-center justify-center gap-2 transition-all cursor-pointer active:scale-95 shadow-sm`}
            >
              {copied ? (
                <>
